...
class node {
    ...
    public:
        ...
        virtual bool return_dst_node()= 0;
...
class TRA_switch: public node {
        ...
        map<unsigned int, unsigned int> routing_table; // <dst, src>
        map<unsigned int, bool> flag;//consider whether have receive ctrl_packet
        map<unsigned int, unsigned int> node_cur_count;//<dst, count>
    ...
    public:
        ...
        unsigned int get_next_id(unsigned int dst){
            return routing_table[dst];
        }
        map<unsigned int, unsigned int> get_routing_table(){
            return routing_table;
        }
        unsigned int get_routing_table_size(){
            return routing_table.size();
        }
        void set_routing_table(unsigned int dst,unsigned int src){
            routing_table[dst] = src;
        }
        unsigned int get_count(unsigned int dst){
            return node_cur_count[dst];
        }
        void set_node_count(unsigned int dst, unsigned count){
            node_cur_count[dst] = count;
        }
        bool get_flag(unsigned int dst){
            return flag[dst];
        }
        void set_flag_ture(unsigned int dst){
            flag[dst] = true;
        }
        void cout_table(unsigned int dst){
            cout << dst << " " << routing_table[dst] << endl;
        }
...
class SDN_switch: public node {
        map<unsigned int, vector<unsigned int>> one_hop_neighbors; // you can use this variable to record the node's 1-hop neighbors 
        map<unsigned int, bool> flag;
        map<unsigned int, vector<pair<unsigned int, double>>> routing_table; //<dst, <next ID, portion>>
        map<unsigned int, unsigned int> node_cur_count;//<dst, count>
        map<unsigned int, unsigned int> OSPF_routing_table;
        //bool hi; // this is used for example; you can remove it when doing hw2

        bool dst_node = false;//whether is dst_node
    protected:
    ...
    public:
        ...
        void add_one_hop_neighbor (unsigned int dst, unsigned int n_id) { one_hop_neighbors[dst].push_back(n_id); }
        double get_one_hop_neighbor_num (unsigned int dst) { return one_hop_neighbors[dst].size(); }
        vector<unsigned int> get_one_hop_neighbor(unsigned int dst){return one_hop_neighbors[dst];}
        unsigned int get_next_id(unsigned int dst){
            return OSPF_routing_table[dst];
        }
        map<unsigned int, vector<pair<unsigned int, double>>> get_routing_table(){
            return routing_table;
        }
        ...
        void set_routing_table(unsigned int dst,unsigned int next_id, double portion){
            routing_table[dst].push_back({next_id, portion});
        }
        unsigned int get_count(unsigned int dst){
            return node_cur_count[dst];
        }
...
class SDN_controller: public node { 
        map<unsigned int, unsigned int> rounting_table; // <dst, src>
        map<unsigned int, bool> flag;//consider whether have received ctrl_packet
        map<unsigned int, unsigned int> node_cur_count;//<dst, count>
        //bool hi; // this is used for example; you can remove it when doing hw2
        bool dst_node = false;//whether is dst_node
    protected:
    ...
    public:
        ...
        // void add_one_hop_neighbor (unsigned int n_id) { one_hop_neighbors[n_id] = true; }
        // unsigned int get_one_hop_neighbor_num () { return one_hop_neighbors.size(); }
        unsigned int get_rounting_table(unsigned int dst){
            return rounting_table[dst];
        }
        void set_routing_table(unsigned int dst,unsigned int src){
            rounting_table[dst] = src;
        }
        unsigned int get_count(unsigned int dst){
            return node_cur_count[dst];
        }
        ...
        void cout_table(unsigned int dst){
            cout << dst << " " << rounting_table[dst] << endl;
        }
...
void TRA_switch::recv_handler (packet *p){
    ...
    if(p->type() == "TRA_ctrl_packet"){
        ...
        unsigned int src_node = p3->getHeader()->getSrcID();
        //if is 
        if(src_node == getNodeID() && dst_node == false){
            dst_node = true;
        }
        //never received ctrl_packet
        if(!flag[src_node]){
            this->set_node_count(src_node, l3->getCounter());
            this->set_routing_table(src_node, p->getHeader()->getPreID());
            flag[src_node] = true;
        ...
        //have received ctrl_packet
        else{
            unsigned int cur_preID = routing_table[src_node];
            unsigned int new_preID = p3->getHeader()->getPreID();
            unsigned int cur_count = node_cur_count[src_node];
            unsigned int new_count = l3->getCounter();
            
            if(l3->getCounter() > node_cur_count[src_node])
                return;
            //update the routing_table
            if(l3->getCounter() < this->get_count(src_node)){
                node_cur_count[src_node] = new_count;
                routing_table[src_node] = new_preID;
            ...
            //if count is equal, than compare the ID and choose the smaller one
            if(l3->getCounter() == this->get_count(src_node)){
                if(this->get_next_id(src_node) > p->getHeader()->getPreID()){
                    node_cur_count[src_node] = new_count;
                    routing_table[src_node] = new_preID;
        ...
        //never received the ctrl_packet => drop the data
        if(routing_table.find(p->getHeader()->getDstID()) == routing_table.end())
            return;
        //already to the destination
        if(getNodeID() == p3->getHeader()->getDstID()){
            return;
        } 
        p3->getHeader()->setPreID ( getNodeID() );
        p3->getHeader()->setNexID(routing_table[p3->getHeader()->getDstID()]);
    ...
    void SDN_switch::recv_handler (packet *p){
        ...
        if(p->type() == "SDN_ctrl_packet"){
            ...
            set_routing_table(l3->getMatID(), l3->getActID(), l3->getPer());
        }
            ...
            unsigned int dst_node = p3->getHeader()->getDstID();
            //if never received ctrl_packet
            if (node_cur_count.find(dst_node) == node_cur_count.end())
                return;
            //if is dst
            if(dst_node == getNodeID ()) return;

            int one_nop_neighbor_num = this->get_one_hop_neighbor_num(dst_node);
            unsigned int flow_size = p3->getSize();
            //for each one_hop_neighbor, send the packet to them
            for(int i = 0;i < one_nop_neighbor_num; i++){

                p3->getHeader()->setPreID(getNodeID());
                p3->getHeader()->setNexID(this->get_routing_table()[dst_node][i].first);
                p3->getHeader()->setDstID(dst_node);
                p3->setSize(this->get_routing_table()[dst_node][i].second * flow_size);
                
                send_handler(p3);
            }
        }
        else if(p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller
            
            TRA_ctrl_packet *p3 = nullptr;
            p3 = dynamic_cast<TRA_ctrl_packet*> (p);
            TRA_ctrl_payload *l3 = nullptr;
            l3 = dynamic_cast<TRA_ctrl_payload*> (p->getPayload());
            unsigned int src_node = p3->getHeader()->getSrcID();

            //if is dst_node
            if((p3->getHeader()->getSrcID() == getNodeID()) && (dst_node == false)){
                dst_node = true;
            }
            //never received ctrl_packet
            if(!(flag[src_node])){
                ...
                flag[src_node] = true;
                node_cur_count[src_node] = l3->getCounter();
                OSPF_routing_table[src_node] = p3->getHeader()->getPreID();
                p->getHeader()->setPreID ( getNodeID());
                p->getHeader()->setNexID ( BROCAST_ID );
                p->getHeader()->setDstID ( BROCAST_ID );
                l3->increase();
                send_handler(p);
            }
            //have received ctrl_packet
            else{
                unsigned int cur_preID = OSPF_routing_table[src_node];
                unsigned int new_preID = p3->getHeader()->getPreID();
                unsigned int cur_count = node_cur_count[src_node];
                unsigned int new_count = l3->getCounter();
                ...
                if(l3->getCounter() < this->get_count(src_node)){
                    node_cur_count[src_node] = new_count;
                    OSPF_routing_table[src_node] = new_preID;
    ...
    void SDN_controller::recv_handler (packet *p){
        ...
        if(p->type() == "SDN_invoke_packet"){
            ...
            map<unsigned int, bool> con_neighbor = this->getPhyNeighbors();
            
            //just want to take the dst_node_list
            TRA_switch *temp = nullptr;
            for(int i = 0; i < node_num;i++){
                if(!con_neighbor[i]){
                    temp = dynamic_cast<TRA_switch*>(id_to_node(i));
                    break;
                }
            }

            map<unsigned int, unsigned int> m = temp->get_routing_table();
            //for each dst_node
            for(auto it = m.begin(); it != m.end(); it++){
                int dst_node = it->first;
                //for each SDN_node
                for(int i = 0; i < node_num; i++){
                    if(con_neighbor[i]){
                        SDN_switch *SDN_node = nullptr;
                        SDN_node = dynamic_cast<SDN_switch*>(id_to_node(i));
                        //cout << "SDN_node : "<< i << "  count : " << SDN_node->get_count(dst_node) <<endl;
                        map<unsigned int, bool> SDN_neighbor = SDN_node->getPhyNeighbors();
                        //for each SDN_node's neighbor
                        for(int j = 0; j < node_num; j++){
                            //SDN_node == dst_node
                            if(i == dst_node){
                                SDN_node->add_one_hop_neighbor(dst_node, i);
                                break;
                            }
                            if(!SDN_neighbor[j]) continue; // isn't SDN's neighbor
                            //is SDN_node
                            if(con_neighbor[j]){
                                SDN_switch *cur_node = nullptr;
                                cur_node = dynamic_cast<SDN_switch*>(id_to_node(j));
                                
                                //compare the counter, can pass to the smaller count SDN_node
                                if(SDN_node->get_count(dst_node) > cur_node->get_count(dst_node)){
                                    SDN_node->add_one_hop_neighbor(dst_node, j);
                                }
                            }
                            //is TRA_node
                            else{
                                TRA_switch *cur_node = nullptr;
                                cur_node = dynamic_cast<TRA_switch*>(id_to_node(j));
                                //compare the counter
                                if(SDN_node->get_count(dst_node) >= cur_node->get_count(dst_node)){
                                    SDN_node->add_one_hop_neighbor(dst_node, j);
                                }
                        ...
                        //average send to all the one_hop_neighbor
                        double one_hop_neighbor_num = SDN_node->get_one_hop_neighbor_num(dst_node);
                        double portion_all = 100.0/one_hop_neighbor_num;                  
                        double portion_last = 100.0 - (one_hop_neighbor_num - 1) * portion_all;
                        
                        //send SDN_ctrl_packet
                        for(int k = 0; k < (int)one_hop_neighbor_num; k++){
                            if(k != one_hop_neighbor_num - 1)
                                SDN_ctrl_packet_event(this->getNodeID(), i, dst_node, SDN_node->get_one_hop_neighbor(dst_node)[k], portion_all*0.01);
                            else
                                SDN_ctrl_packet_event(this->getNodeID(), i, dst_node, SDN_node->get_one_hop_neighbor(dst_node)[k], portion_last*0.01);
                        }
                    }
                }
            }
            ...
            p3 = dynamic_cast<SDN_ctrl_packet*> (p);
            SDN_ctrl_payload *l3 = nullptr;
            l3 = dynamic_cast<SDN_ctrl_payload*> (p3->getHeader());
...
int main(){
    ...
    int Nodes, SDN_Nodes, Dsts, Links, Pairs, SimTime, InvokeTime;
    cin >> Nodes >> SDN_Nodes >> Dsts >> Links >> Pairs >> SimTime >> InvokeTime;
    int SDN_list[SDN_Nodes];
    ...
    for(int i = 0; i < Dsts; i++){
        int Broad_ID, BroadcastTime;
        cin >> Broad_ID >> BroadcastTime;
        TRA_ctrl_packet_event(Broad_ID, BroadcastTime);
    }
    ...
    for(int i = 0;i < Pairs; i++){
        ...
        data_packet_event(Src, Dst, FlowSize, StartTime);
    }
    ...
    for(int i = 0; i < SDN_Nodes; i++){
        if(i == SDN_Nodes -1)
            cout << SDN_list[i] << endl;
        else
            cout << SDN_list[i] << " ";
    }
    for(int i = 0; i < Nodes; i++){
        cout << i << endl;
        node *curnode = node::id_to_node(i);
        if(curnode->type() == "SDN_switch"){
            for(int j = 0; j < Dsts; j++){
                cout << DstID_list[j] << " ";
                SDN_switch *SDN_node = nullptr;
                SDN_node = dynamic_cast<SDN_switch*>(curnode);
                
                
                for(int k = 0; k < SDN_node->get_one_hop_neighbor_num(DstID_list[j]); k++){
                    if(k != SDN_node->get_one_hop_neighbor_num(DstID_list[j]) - 1){
                        cout << SDN_node->get_routing_table()[DstID_list[j]][k].first << " " << SDN_node->get_routing_table()[DstID_list[j]][k].second*100 << "% ";
                    }
                    else{
                        cout << SDN_node->get_routing_table()[DstID_list[j]][k].first << " " << SDN_node->get_routing_table()[DstID_list[j]][k].second*100 << "%" << endl;
                    }
                }
            }
        }
        else if(curnode->type() == "TRA_switch"){
            TRA_switch *TRA_node = nullptr;
            TRA_node = dynamic_cast<TRA_switch*>(curnode);
            for(int j = 0; j < Dsts; j++){
                cout << DstID_list[j] << " " << TRA_node->get_routing_table()[DstID_list[j]] << endl;
            }
        }
    }
    return 0;
